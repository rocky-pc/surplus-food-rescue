...
-- Complete clean setup - drops and recreates everything
-- ============================================================
...
(8, 'dairy', 'Cheese blocks and butter', 20, 100, DATE_SUB(NOW(), INTERVAL 2 HOUR), DATE_ADD(NOW(), INTERVAL 168 HOUR), TRUE, TRUE, 'available'),
(9, 'cooked_food', 'Hyderabadi biryani and haleem', 40, 220, DATE_SUB(NOW(), INTERVAL 30 MINUTE), DATE_ADD(NOW(), INTERVAL 3 HOUR), TRUE, FALSE, 'available'),
(10, 'bakery', 'Assorted cakes and pastries', 15, 100, DATE_SUB(NOW(), INTERVAL 1 HOUR), DATE_ADD(NOW(), INTERVAL 10 HOUR), FALSE, TRUE, 'available'),
(11, 'cooked_food', 'Chettinad meals with fish curry', 25, 150, DATE_SUB(NOW(), INTERVAL 45 MINUTE), DATE_ADD(NOW(), INTERVAL 5 HOUR), TRUE, FALSE, 'available'),
(12, 'grains', 'Pulses and lentils', 80, 0, DATE_SUB(NOW(), INTERVAL 5 HOUR), DATE_ADD(NOW(), INTERVAL 8760 HOUR), FALSE, TRUE, 'available'),
...
SELECT batch_id, status FROM donation_batches WHERE batch_id = 1 AND status = 'available' FOR UPDATE;

UPDATE donation_batches SET status = 'reserved' WHERE batch_id = 1 AND status = 'available';

UPDATE volunteers SET is_available = FALSE WHERE volunteer_id = 1 AND is_available = TRUE;
